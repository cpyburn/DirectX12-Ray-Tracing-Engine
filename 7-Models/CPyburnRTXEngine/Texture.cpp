...
//		Microsoft::WRL::ComPtr<ID3D12Resource> textureUploadResource;
//
...
//
//		const UINT64 uploadBufferSize = GetRequiredIntermediateSize(texture.Get(), 0, 1); // +D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT;
//
//		DX::ThrowIfFailed(d3dDevice->CreateCommittedResource(
...
//
//		texture->SetName(wFileName.c_str());
//		textureUploadResource->SetName(wFileName.c_str());
//
...
//
//			UpdateSubresources(commandList, texture.Get(), textureUploadResource.Get(), 0, 0, 1, &textureData);
//
//			CD3DX12_RESOURCE_BARRIER srvBufferResourceBarrier =
//				CD3DX12_RESOURCE_BARRIER::Transition(texture.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
...
//			d3dDevice->CreateShaderResourceView(texture.Get(), &srvDesc, cbvCpuHandle);
//
...
//			Texture::m_textures.insert(std::pair<UINT, Microsoft::WRL::ComPtr<ID3D12Resource> >(heapPostion, texture));
//			Texture::m_texturesUpload.insert(std::pair<UINT, Microsoft::WRL::ComPtr<ID3D12Resource> >(heapPostion, textureUploadResource));
//			if (didLock)
//			{
//				m_mutex.unlock();
//			}
