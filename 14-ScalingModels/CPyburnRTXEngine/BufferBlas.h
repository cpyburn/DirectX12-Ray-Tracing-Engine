...
namespace CPyburnRTXEngine
{
    ...
    public:
        ...
        void InitBlas(ID3D12Device5* m_d3dDevice, const UINT& count, const Microsoft::WRL::ComPtr<ID3D12Resource> vertexBuffer, ID3D12GraphicsCommandList4* commandList, Microsoft::WRL::ComPtr<ID3D12Resource> indicesBuffer = nullptr, UINT indicesCount = 0)
        {
            ...
            if (!m_scratch.Get())
            {
                DX::ThrowIfFailed(m_d3dDevice->CreateCommittedResource(&heapPropertiesDefault, D3D12_HEAP_FLAG_NONE, &m_bufDesc, D3D12_RESOURCE_STATE_COMMON, nullptr, IID_PPV_ARGS(&m_scratch)));
        ...
        //static Microsoft::WRL::ComPtr<ID3D12Resource> CreateStaticBlas(DX::DeviceResources* deviceResources, const UINT& count, const Microsoft::WRL::ComPtr<ID3D12Resource> vertexBuffer, Microsoft::WRL::ComPtr<ID3D12Resource> indicesBuffer = nullptr, UINT indicesCount = 0)
        //{
        //    ID3D12GraphicsCommandList4* commandList = deviceResources->GetCurrentFrameResource()->GetCommandList(0);
        ...
        //    deviceResources->GetCurrentFrameResource()->ResetCommandList(0, nullptr);
